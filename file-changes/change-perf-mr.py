...
# Gather both addon change and bootstrap performance measures from telemetry
    ...
    for addonID, details in x.iteritems():
      ...
      for measure in MEASURES:
        if measure in details:
          result[measure] = {details[measure] / 10: 1}
...
def reduce(k, v, cx):
    ...
    for val in v:
      ...
      for measure in MEASURES:
        if measure in val:
          result[measure].update(val[measure])

    # Convert Counters to plain dicts
    for measure in MEASURES:
      counts = result.pop(measure, {})
      if len(counts) > 0:
        result[measure] = dict(counts)
    cx.write(k, result)
